...
<div id="tab-members-cards-accounts" class="tab-pane">
	...
	<table class="table table-striped">
		<thead>
			<tr>
				...
				<th>Card / <?php echo ((TSP_ACH_ENABLED == 1) ? '/ ACH' : ''); ?></th>
				<th>Description</th>
				<th>Auto Payment</th>
		...
		<?php if (!empty($payment_sources)) : ?>
			...
			<tr>
				<td class="text-center">
						...
						<?php echo (($payment_source['default'] == '1') ? 'checked="checked"' : ''); ?>  tsp_boot_tab = "tab-members-cards-accounts"/>
				</td>
				...
				<select name="auto_payment"  class="form-control" name="auto_payment"
						tsp_obj="gateway" tsp_method="SubmitAutopay" tsp_id="<?php echo $payment_source['id']; ?>" tsp_boot_tab = "tab-members-cards-accounts" tsp_auto_payment = "<?php echo $payment_source['auto_payment']; ?>"
				...
				<?php if (TSP_OC_REMOVE_PAYMENT_SOURCE == '1') : ?>
				<td class="text-center">
					<a href="#tab-members-cards-accounts" tsp_obj="gateway" tsp_method="Delete"  tsp_id="<?php echo $payment_source['id']; ?>" onclick="if (confirm('Are you sure you want to delete Card/ACH?')) { tsp_ajax_click(this) };return false;" title="Delete Card/ACH" class="btn btn-danger btn-sm"><i class="fa fa-minus-circle"></i> Remove</a>
				</td>
				<?php endif; ?>
			</tr>
			<?php endforeach; ?>
	...
	<?php if (TSP_OC_ALLOW_CARD_PAYMENTS) : ?>
		<div class="form-group">
			<a href="#gateway-card" class="tsp_ajax_tab btn btn-primary tsp_ajax_tab" onclick="return false;">Add Card</a>
		</div>
	<?php endif; ?>
	<?php if (TSP_ACH_ENABLED && TSP_OC_ALLOW_ACH_PAYMENTS) : ?>
		<div class="form-group">
			<a href="#gateway-account" class="tsp_ajax_tab btn btn-primary tsp_ajax_tab" onclick="return false;">Add ACH</a>
		</div>
	<?php endif; ?>
	</div>
</div>
