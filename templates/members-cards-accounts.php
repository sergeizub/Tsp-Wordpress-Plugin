		...
		<? if (!empty($payment_sources)) : ?>
			...
			<tr>
				<td class="text-center">
						...
						<?php echo (($payment_source['default'] == '1') ? 'checked="checked"' : ''); ?>  dsm_boot_tab = "tab-members-cards-accounts"/>
				</td>
				...
				<select name="auto_payment"  class="form-control" name="auto_payment"
						dsm_obj="gateway" dsm_method="SubmitAutopay" dsm_id="<?php echo $payment_source['id']; ?>" dsm_boot_tab = "tab-members-cards-accounts" dsm_auto_payment = "<?php echo $payment_source['auto_payment']; ?>"
				...
				<?php if (DSM_OC_REMOVE_PAYMENT_SOURCE == '1') : ?>
				<td class="text-center">
					<a href="#tab-members-cards-accounts" dsm_obj="gateway" dsm_method="Delete"  dsm_id="<?php echo $payment_source['id']; ?>" onclick="if (confirm('Are you sure you want to delete Card/ACH?')) { dsm_ajax_click(this) };return false;" title="Delete Card/ACH" class="btn btn-danger btn-sm"><i class="fa fa-minus-circle"></i> Remove</a>
				</td>
				<?php endif; ?>
			</tr>
			<? endforeach; ?>
