...
if(!empty($_SESSION['dsm_client_attrs']))
	$classes_list = App::GetClient()->GetController('classes')->GetClasses((array)$_SESSION['dsm_client_attrs'] + json_decode(str_replace('\"','"',$_REQUEST['filter']),true));
else
	$classes_list = App::GetClient()->GetController('classes')->GetClasses(json_decode(str_replace('\"','"',$_REQUEST['filter']),true));
$i = 0;
